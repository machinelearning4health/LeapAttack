...
def leapattack(fuzz_val, top_k_words, qrs, sample_index, text_ls, true_label,
            ...
            for t in range(100): 
                ...
                if np.sum(get_attack_result([x_t], predictor, orig_label, batch_size))>0  and (num_changed == 1):
                    return ' '.join(x_t), 1, len(changed_indices), \
                        orig_label, torch.argmax(predictor([x_t])), qrs, x_t_sim, random_sim
                ...
                if np.sum(pr)>0:
                    ...
                    if (sim_new > best_sim) and (np.sum(get_attack_result([x_tilde], predictor, orig_label, batch_size))>0):
                        best_attack = x_tilde[:]
                        best_sim = sim_new
...
def main():
    ...
    log_file = "results_hard_label/"+args.target_model+"/"+args.target_dataset+"/leapattack_log.txt"
    result_file = "results_hard_label/"+args.target_model+"/"+args.target_dataset+"/leapattack_results_final.csv"
    ...
    with open(os.path.join(args.output_dir, args.target_model+'leapattack_adversaries.txt'), 'w') as ofile:
