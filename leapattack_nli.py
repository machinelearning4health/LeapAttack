...
def leapattack(fuzz_val, top_k_words, qrs, sample_index, hypotheses, premise, true_label,
    ...
    if true_label != orig_label:
        ...
        if flag == 1:
            ...
            for t in range(100): 
                ...
                if np.sum(get_attack_result([x_t], premise, predictor, orig_label, batch_size))>0  and (num_changed == 1):
                    qrs+=1
                    return ' '.join(x_t), 1, len(changed_indices), \
                        orig_label, torch.argmax(predictor({'premises':[premise], 'hypotheses': [x_t]})), qrs, x_t_sim, random_sim


                if np.sum(get_attack_result([x_t], premise, predictor, orig_label, batch_size))>0 and  (x_t_sim  > best_sim):
                    qrs+=1
                    best_attack = x_t[:]
                    best_sim = x_t_sim
                ...
                for idx in words_perturb_doc_idx:
                    x_t_adv_embed.append([float(num) for num in embed_content[word_idx_dict[x_t[idx]]].strip().split()[1:]])
                ...
                perturb_strength_order = np.argsort(-x_t_perturb_dist[nonzero_ele])
                ...
                for perturb_idx in range(len(nonzero_ele)):
                    ...
                    orig_word_emb=np.asarray([float(num) for num in embed_content[word_idx_dict[orig_word]].strip().split()[1:]])
                    for _ in range(n_sample):
                        ...
                        qrs+=1
                        syn_vector_set.append(np.asarray([float(num) for num in embed_content[word_idx_dict[replacement]].strip().split()[1:]])-orig_word_emb)

                    grad_dir=1/(n_sample-1)*np.matmul((np.asarray(syn_sim_set)-sum(syn_sim_set)/len(syn_sim_set)), np.asarray(syn_vector_set))
                    if np.sum(grad_dir)==0:
                        replacement = orig_word
                    else:
                        cos_sim_set=[]
                        for syn in synonyms_all[perturb_word_idx][1]:
                            syn_feat = np.asarray([float(num) for num in embed_content[word_idx_dict[syn]].strip().split()[1:]])
                            cos_sim_set.append(spatial.distance.cosine((syn_feat-orig_word_emb).tolist(),grad_dir.tolist()))

                        perturb_syn_order = cos_sim_set.index(min(cos_sim_set))
                        replacement = synonyms_all[perturb_word_idx][1][perturb_syn_order]
                    
                    x_tilde[synonyms_all[perturb_word_idx][0]] = replacement
                ...
                if np.sum(pr)>0:
                    ...
                    if (sim_new > best_sim) and (np.sum(get_attack_result([x_tilde], premise, predictor, orig_label, batch_size))>0):
                        best_attack = x_tilde[:]
                        best_sim = sim_new
...
def main():
    ...
    log_file = "results_nli_hard_label/"+args.target_model+"/"+args.target_dataset+"/leapattack_sign_update_log.txt"
    result_file = "results_nli_hard_label/"+args.target_model+"/"+args.target_dataset+"/leapattack_sign_update_results_final.csv"
    ...
    for idx, premise in enumerate(data['premises']):
        ...
        if args.perturb_ratio == 0:
            ...
            new_label, num_queries, sim, random_sim = leapattack(args.fuzz,args.top_k_words,args.allowed_qrs,
                                            ...
                                            batch_size=args.batch_size,embed_func = args.counter_fitting_embeddings_path)
        if true_label != orig_label:
            orig_failures += 1
        else:
    ...
    with open(os.path.join(args.output_dir, 'leapattack_adversaries.txt'), 'w') as ofile:
